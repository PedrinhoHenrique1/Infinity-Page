<!DOCTYPE html>
<html lang="pt-br">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title> Infinity Page </title>
    <link rel="stylesheet" href="../css/inicial.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
</head>

<body>

    <button class="menu-btn" id="toggle-btn">
        <i class="fa-solid fa-bars"></i>
    </button>

    <div id="sidebar" class="sidebar">
        <div class="sidebar-header">
            <h2>Menu</h2>
        </div>
        <ul>
            <li><a href="inicial.html"><i class="fa-solid fa-house"></i> Início </a></li>
            <li><a href="pedidos.html"><i class="fa-solid fa-bag-shopping"></i> Meus pedidos </a></li>
            <li><a href="favoritos.html"><i class="fa-solid fa-star"></i> Favoritos </a></li>
            <li><a href="carrinho.html"><i class="fa-solid fa-cart-shopping"></i> Carrinho </a></li>
            <li><a href="trocas.html"><i class="fa-solid fa-arrows-rotate"></i> Trocas </a></li>
            <li><a href="../Login/login.html"><i class="fa-solid fa-door-open"></i> Sair </a></li>
        </ul>
    </div>

    <header>
        <div class="logo">
            <img src="../images/logo.png" width="150px">
        </div>
        <div class="texto">
            <input type="search" id="buscaItem" placeholder="O que você busca hoje?" oninput="filtrarItens()">
        </div>
        <div class="trade">
            <a href="../pages/trocas.html"> Trocas </a>
        </div>
        <div class="atendimento">
            <a href="atendimento.html"> Atendimento ao Cliente </a>
        </div>
        <div class="icons">
            <div class="favorite">
                <a href="favoritos.html">
                    <i class="fa-solid fa-heart fa-3x" style="color: #ffffff;"></i>
                </a>
            </div>
            <div class="cart">
                <a href="carrinho.html">
                    <i class="fa-solid fa-cart-shopping fa-3x" style="color: #ffffff;"></i>
                </a>
            </div>
            <div id="area-login">
                <button id="btn-login" onclick="fazerLogin()">Login</button>
            </div>
        </div>
    </header>
    <main id="conteudo-principal">
        <nav>
            <ul>
                <li><a href="#novidades"> Novidades </a></li>
                <li><a href="#esportes"> Esportes </a></li>
                <li><a href="#dc_comics"> DC Comics </a></li>
                <li><a href="#marvel"> Marvel </a></li>
            </ul>
        </nav>

        <nav2>
            <button class="dc-button" data-category="ficcao"> Ficção </button>
            <button class="dc-button" data-category="romance"> Romance </button>
            <button class="dc-button" data-category="comedia"> Comédia </button>
            <button class="dc-button" data-category="quadrinhos"> Quadrinhos </button>
            <button class="dc-button" data-category="academico"> Acadêmico </button>
            <button class="dc-button" data-category="colecionaveis"> Colecionáveis </button>
        </nav2>

        <section class="conteudo-categoria" data-category="ficcao" hidden>
            <h2>Ficção</h2>
            <div class="products-container">
                <!-- Cards de produtos de Ficção -->
                <div class="product">
                    <img src="../images/livro-ficcao1.png" alt="Livro Ficção 1">
                    <h3>Livro Ficção 1</h3>
                    <p>R$ 29,90</p>
                    <button class="buy-button">Comprar</button>
                </div>
                <!-- Adicione mais cards para esta categoria -->
            </div>
        </section>

        <section class="conteudo-categoria" data-category="romance" hidden>
            <h2>Romance</h2>
            <div class="products-container">
                <div class="product">
                    <img src="../images/duque-implacavel.png" alt="Livro Romance 1">
                    <h3>O Duque Implacável</h3>
                    <p>R$ 39,90</p>
                    <button class="buy-button">Comprar</button>
                </div>
                <div class="product">
                    <img src="../images/ayrton-senna.png" alt="Livro Romance 1">
                    <h3>Veloz como o Vento </h3>
                    <p>R$ 58,00</p>
                    <button class="buy-button">Comprar</button>
                </div>
            </div>
        </section>

        <div id="conteudo-site">
            <section class="category-section" data-category="ficcao" hidden>Conteúdo de Ficção</section>
            <section class="category-section" data-category="romance" hidden>Conteúdo de Romance</section>
            <section class="category-section" data-category="comedia" hidden>Conteúdo de Comédia</section>
            <section class="category-section" data-category="quadrinhos" hidden>Conteúdo de Quadrinhos</section>
            <section class="category-section" data-category="academico" hidden>Conteúdo Acadêmico</section>
            <section class="category-section" data-category="colecionaveis" hidden>Conteúdo de Colecionáveis</section>

            <section>
                <h2> Novidades </h2>
            </section>

            <section id="secao-produtos">
                <div class="products-container">
                    <div class="product" onclick="mostrarDetalhe('1')">
                        <img src="../images/coringa.png" alt="Livro 1">
                        <h3> Batman: A Piada Mortal </h3>
                        <p>R$ 30,90</p>
                        <button class="buy-button">Comprar</button>
                    </div>

                    <div class="product" onclick="mostrarDetalhe('2')">
                        <img src="../images/silver-fang.png" alt="Livro 2">
                        <h3>One-Punch Man Vol. 4</h3>
                        <p>R$ 42,90</p>
                        <button class="buy-button">Comprar</button>
                    </div>

                    <div class="product" onclick="mostrarDetalhe('3')">
                        <img src="../images/vingadores.png" alt="Livro 3">
                        <h3>Os Vingadores 17/74</h3>
                        <p>R$ 49,99</p>
                        <button class="buy-button">Comprar</button>
                    </div>

                    <div class="product" onclick="mostrarDetalhe('4')">
                        <img src="../images/tio-patinhas.png" alt="Livro 3">
                        <h3> Tio Patinhas E A Última Aventura </h3>
                        <p>R$ 129,99</p>
                        <button class="buy-button">Comprar</button>
                    </div>
                </div>
            </section>
    </main>

    <section id="detalhe-produto-1" class="produto-detalhe" style="display: none;">
        <div class="produto-detalhe-container">
            <button class="produto-detalhe-voltar" onclick="voltarParaHome()">← Voltar</button>
            <div class="produto-detalhe-box">
                <div class="produto-detalhe-img">
                    <img src="../images/coringa.png" alt="Batman: A Piada Mortal">
                </div>
                <div class="produto-detalhe-info">
                    <h2>Batman: A Piada Mortal</h2>
                    <p class="produto-detalhe-subtitulo">Por Alan Moore e Brian Bolland</p>
                    <div class="produto-detalhe-avaliacao">
                        <span>★★★★☆</span>
                        <small>(254 avaliações)</small>
                    </div>
                    <p class="produto-detalhe-preco">R$ 30,90</p>
                    <p class="produto-detalhe-descricao">
                        Uma das obras mais impactantes do universo do Batman. A origem do Coringa recontada de forma
                        sombria e envolvente.
                    </p>
                    <ul class="produto-detalhe-tecnico">
                        <li>Editora: DC Comics</li>
                        <li>Idioma: Português</li>
                        <li>Encadernação: Capa dura</li>
                        <li>Número de páginas: 64</li>
                        <li>Data de lançamento: 10/2020</li>
                    </ul>
                    <div class="produto-detalhe-compra">
                        <label for="quantidade">Quantidade:</label>
                        <input type="number" id="quantidade" name="quantidade" min="1" value="1">
                        <a href="pagamento.html">
                            <button class="produto-detalhe-btn"> Comprar </button>
                        </a>
                        <button class="produto-detalhe-btn"> Adicionar ao carrinho</button>
                    </div>
                </div>
            </div>

            <div class="produto-relacionado">
                <h3>Produtos Relacionados</h3>
                <div class="produto-relacionado-lista">
                    <!-- Cards com produtos relacionados -->
                </div>
            </div>
        </div>
    </section>
    <section id="detalhe-produto-2" class="produto-detalhe" style="display: none;">
        <div class="produto-detalhe-container">
            <button class="produto-detalhe-voltar" onclick="voltarParaHome()">← Voltar</button>
            <div class="produto-detalhe-box">
                <div class="produto-detalhe-img">
                    <img src="../images/silver-fang.png" alt="One-Punch Man Vol. 4">
                </div>
                <div class="produto-detalhe-info">
                    <h2> One-Punch Man Vol. 4 </h2>
                    <p class="produto-detalhe-subtitulo">Por Alan Moore e Brian Bolland</p>
                    <div class="produto-detalhe-avaliacao">
                        <span>★★★★☆</span>
                        <small>(126 avaliações)</small>
                    </div>
                    <p class="produto-detalhe-preco">R$ 42,90</p>
                    <p class="produto-detalhe-descricao">
                        O quarto volume da série que redefiniu o gênero de super-heróis. A luta contra o poderoso Garou
                        começa!
                    </p>
                    <ul class="produto-detalhe-tecnico">
                        <li>Editora: Panini</li>
                        <li>Idioma: Português</li>
                        <li>Encadernação: Capa dura</li>
                        <li>Número de páginas: 124</li>
                        <li>Data de lançamento: 09/2023</li>
                    </ul>
                    <div class="produto-detalhe-compra">
                        <label for="quantidade">Quantidade:</label>
                        <input type="number" id="quantidade" name="quantidade" min="1" value="1">
                        <button class="produto-detalhe-btn"> Comprar </button>
                        <button class="produto-detalhe-btn">Adicionar ao carrinho</button>
                    </div>
                </div>
            </div>

            <div class="produto-relacionado">
                <h3>Produtos Relacionados</h3>
                <div class="produto-relacionado-lista">
                    <!-- Cards com produtos relacionados -->
                </div>
            </div>
        </div>
    </section>
    <section id="detalhe-produto-3" class="produto-detalhe" style="display: none;">
        <div class="produto-detalhe-container">
            <button class="produto-detalhe-voltar" onclick="voltarParaHome()">← Voltar</button>
            <div class="produto-detalhe-box">
                <div class="produto-detalhe-img">
                    <img src="../images/vingadores.png" alt="Os Vingadores 17/74">
                </div>
                <div class="produto-detalhe-info">
                    <h2>Os Vingadores 17/74</h2>
                    <p class="produto-detalhe-subtitulo">Por Roy Thomas e John Buscema</p>
                    <div class="produto-detalhe-avaliacao">
                        <span>★★★★☆</span>
                        <small>(198 avaliações)</small>
                    </div>
                    <p class="produto-detalhe-preco">R$ 49,99</p>
                    <p class="produto-detalhe-descricao">
                        Uma edição clássica dos Vingadores, trazendo aventuras épicas e momentos marcantes da equipe
                        mais poderosa da Terra.
                    </p>
                    <ul class="produto-detalhe-tecnico">
                        <li>Editora: Marvel Comics</li>
                        <li> Idioma: Português </li>
                        <li>Encadernação: Capa dura</li>
                        <li>Número de páginas: 96</li>
                        <li>Data de lançamento: 05/2022</li>
                    </ul>
                    <div class="produto-detalhe-compra">
                        <label for="quantidade">Quantidade:</label>
                        <input type="number" id="quantidade" name="quantidade" min="1" value="1">
                        <button class="produto-detalhe-btn"> Comprar </button>
                        <button class="produto-detalhe-btn">Adicionar ao carrinho</button>
                    </div>
                </div>
            </div>

            <div class="produto-relacionado">
                <h3>Produtos Relacionados</h3>
                <div class="produto-relacionado-lista">
                    <!-- Cards com produtos relacionados -->
                </div>
            </div>
        </div>
    </section>
    <section id="detalhe-produto-4" class="produto-detalhe" style="display: none;">
        <div class="produto-detalhe-container">
            <button class="produto-detalhe-voltar" onclick="voltarParaHome()">← Voltar</button>
            <div class="produto-detalhe-box">
                <div class="produto-detalhe-img">
                    <img src="../images/tio-patinhas.png" alt="Tio Patinhas E A Última Aventura">
                </div>
                <div class="produto-detalhe-info">
                    <h2>Tio Patinhas E A Última Aventura</h2>
                    <p class="produto-detalhe-subtitulo">Por Don Rosa</p>
                    <div class="produto-detalhe-avaliacao">
                        <span>★★★★★</span>
                        <small>(312 avaliações)</small>
                    </div>
                    <p class="produto-detalhe-preco">R$ 129,99</p>
                    <p class="produto-detalhe-descricao">
                        Uma das histórias mais emocionantes do universo Disney, acompanhando Tio Patinhas em sua última
                        grande aventura.
                    </p>
                    <ul class="produto-detalhe-tecnico">
                        <li>Editora: Abril</li>
                        <li>Idioma: Português</li>
                        <li>Encadernação: Capa dura</li>
                        <li>Número de páginas: 256</li>
                        <li>Data de lançamento: 08/2021</li>
                    </ul>
                    <div class="produto-detalhe-compra">
                        <label for="quantidade"> Quantidade: </label>
                        <input type="number" id="quantidade" name="quantidade" min="1" value="1">
                        <button class="produto-detalhe-btn" href="pagamento.html"> Comprar </button>
                        <button class="produto-detalhe-btn"> Adicionar ao carrinho </button>
                    </div>
                </div>
            </div>
            <div class="produto-relacionado">
                <h3>Produtos Relacionados</h3>
                <div class="produto-relacionado-lista">
                    <!-- Cards com produtos relacionados -->
                </div>
            </div>
        </div>
    </section>
    </div>


    <script>
        // Botão lateral
        const toggleBtn = document.getElementById('toggle-btn');
        const sidebar = document.getElementById('sidebar');

        toggleBtn.addEventListener('click', () => {
            sidebar.classList.toggle('active');
        });

        // Corrige campo de busca
        function filtrarItens() {
            const termo = document.querySelector("input[type='search']").value.toLowerCase();
            const itens = document.querySelectorAll(".product");

            itens.forEach((item) => {
                const nomeItem = item.textContent.toLowerCase();
                item.style.display = nomeItem.includes(termo) ? "block" : "none";
            });
        }

        function fazerLogin() {
            window.location.href = "../pages/login.html";
        }

        function fazerLogout() {
            localStorage.removeItem("usuarioLogado");
            location.reload();
        }

        function verificarLogin() {
            const usuario = JSON.parse(localStorage.getItem("usuarioLogado"));
            const areaLogin = document.getElementById("area-login");

            if (usuario) {
                areaLogin.innerHTML = `
              <div class="usuario-logado" style="display: flex; align-items: center; gap: 10px;">
                <i class="fa-solid fa-user fa-3x" style="color: #ffffff;"></i>
                <span style="color: white;">${usuario.email.split('@')[0]}</span>
                <button onclick="fazerLogout()" style="background:none;border:none;color:white;cursor:pointer;">Sair</button>
              </div>`;
            } else {
                areaLogin.innerHTML = `<button id="btn-login" onclick="fazerLogin()">Login</button>`;
            }
        }

        document.addEventListener("DOMContentLoaded", () => {
            verificarLogin();
        });

        document.addEventListener("DOMContentLoaded", () => {
            verificarLogin();

            // Sistema de exibição de categorias
            const buttons = document.querySelectorAll('.dc-button');
            const sections = document.querySelectorAll('.category-section');

            buttons.forEach(button => {
                button.addEventListener('click', () => {
                    const category = button.dataset.category;

                    // Esconde todas as seções
                    sections.forEach(section => {
                        section.hidden = section.dataset.category !== category;
                    });

                    // Opcional: destaque o botão ativo
                    buttons.forEach(btn => btn.classList.remove('active'));
                    button.classList.add('active');
                });
            });
        });


        document.addEventListener("DOMContentLoaded", () => {
            // ... outros códigos ...

            const buttons = document.querySelectorAll('.dc-button');
            const conteudoSite = document.getElementById('conteudo-site');
            const categorias = document.querySelectorAll('.conteudo-categoria');

            buttons.forEach(button => {
                button.addEventListener('click', () => {
                    // Esconde todo conteúdo do site menos o header
                    conteudoSite.style.display = 'none';

                    // Esconde todas as categorias
                    categorias.forEach(cat => cat.hidden = true);

                    // Exibe a categoria clicada
                    const category = button.dataset.category;
                    const section = document.querySelector(`.conteudo-categoria[data-category="${category}"]`);
                    if (section) section.hidden = false;

                    // Destaque visual do botão ativo (opcional)
                    buttons.forEach(btn => btn.classList.remove('active'));
                    button.classList.add('active');
                });
            });
        });
    </script>

    <script src="../js/inicial.js"> </script>

</body>

</html>